...
@pytest.mark.parametrize("glm_class_type", ["glm_class", "population_glm_class"])
def test_solver_serialization_mapping_warns_on_mismatch(
    tmp_path, request, glm_class_type
):
    """Mapping overrides saved solver but warns on implementation mismatch."""

    glm_class = request.getfixturevalue(glm_class_type)
    model = glm_class(solver=MappedDummySolver)
    save_path = tmp_path / "model_solver_mapping_mismatch.npz"
    model.save_params(save_path)

    mapping = {"solver": UnregisteredDummySolver}
    with pytest.warns(UserWarning, match="Mismatch between saved solver implementation"):
        loaded = nmo.load_model(save_path, mapping_dict=mapping)

    assert loaded.solver.implementation is UnregisteredDummySolver
    assert loaded.solver.algo_name == MappedDummySolver.__name__
    assert loaded.solver.backend == "unchecked_custom"
