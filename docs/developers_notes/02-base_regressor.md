...
- `solver`: One of the supported solvers in the solver registry or a class that adheres to the `SolverAdapter` interface. Currently the following solvers are available in the registry: "GradientDescent", "BFGS", "LBFGS", "ProximalGradient", "SVRG", and "NonlinearCG".
